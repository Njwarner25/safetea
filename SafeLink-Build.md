# SafeLink — Feature Build

> Supportive, real-world situational messaging for SafeTea's SafeLink screens.
> Tone: Calm, empowering, reassuring. Never fear-based or graphic.

---

## Situational Messaging System

### Primary Hero Message

```
"Riding the train alone? Don't be afraid — SafeLink has you."
```

### Rotating Supportive Lines

These messages rotate on each screen load, providing calm reassurance based on real everyday scenarios:

| # | Message |
|---|---------|
| 1 | "Walking alone? You're not alone." |
| 2 | "Heading home late? Stay connected." |
| 3 | "Something feels off? Trust your instincts — we've got you." |
| 4 | "Commuting late? You don't have to do it alone." |
| 5 | "In an unfamiliar place? SafeLink keeps you connected to people who care." |
| 6 | "Waiting for your ride? We'll wait with you." |
| 7 | "New city, new route? You've got backup." |
| 8 | "Late shift ending? Let someone know you're on your way." |
| 9 | "Parking garage feels empty? You're still connected." |
| 10 | "Long walk to your car? We're with you every step." |

---

## Screen Implementation: `safelink.tsx`
...
Add to `safetea-mobile/app/` as `safelink.tsx`. Register the route in the navigation layout:

```tsx
// In app/_layout.tsx — add inside the Stack
<Stack.Screen name="safelink" options={{ title: 'SafeLink', headerShown: true }} />
```

Add a navigation entry on the dashboard or tabs:

```tsx
// Example dashboard button
<Pressable onPress={() => router.push('/safelink')} style={styles.featureCard}>
  <FontAwesome5 name="link" size={24} color={Colors.coral} />
  <Text style={styles.featureLabel}>SafeLink</Text>
  <Text style={styles.featureDesc}>Share your location with trusted contacts</Text>
</Pressable>
```
...
- Use calm, everyday scenarios (walking, commuting, riding transit)
- Center the user's autonomy ("Trust your instincts", "You've got backup")
- Keep phrasing short and direct
- Rotate messages so the experience feels fresh

### Don't

- Reference specific crimes or violent incidents
- Use graphic or fear-based language
- Imply the user is in danger
- Use statistics about violence on SafeLink screens
...
| Token | Value | Usage |
|-------|-------|-------|
| `Colors.background` | `#1A1A2E` | Screen background |
| `Colors.surface` | `#22223A` | Card backgrounds |
| `Colors.coral` | `#E8A0B5` | Brand accent, buttons, reassurance text |
| `Colors.coralMuted` | `rgba(232,160,181,0.15)` | Message banner background |
| `Colors.success` | `#2ecc71` | Active session border, status chips |
| `Colors.danger` | `#e74c3c` | End session button |
| `Colors.textPrimary` | `#F0D0C0` | Headlines, situation text |
| `Colors.textSecondary` | `#F5E0D5` | Body copy |
| `Colors.textMuted` | `#8080A0` | Footer, privacy note |
...
| File | Purpose |
|------|---------|
| `app/safelink.tsx` | Main SafeLink screen with situational messaging |
| `app/_layout.tsx` | Route registration (add `Stack.Screen`) |
| Dashboard / Tabs | Navigation entry point to SafeLink |

---

*Built for SafeTea. Not deployed — implementation reference only.*
