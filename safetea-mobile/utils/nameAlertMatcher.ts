...
export interface NameAlertEntry {
  id: string;
  userId: string;
  displayName: string;
  searchTerms: string[];
...
export interface NameAlertMatch {
  id: string;
  entryId: string;
  postId: string;
  matchedTerm: string;
 ...
 */
export function matchPost(post: Post, entries: NameAlertEntry[]): NameAlertMatch[] {
  const matches: NameAlertMatch[] = [];
