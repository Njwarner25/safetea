...
router.post('/register', [
    ...
    const avatar_color = colors[Math.floor(Math.random() * colors.length)];
              ...
              try {
                    db.prepare(`
                          INSERT INTO users (id, email, password_hash, display_name, city, state, avatar_initial, avatar_color)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                                    `).run(id, email, password_hash, display_name, city, state, avatar_initial, avatar_color);
      ...
      res.status(201).json({
              ...
              token,
              user: { id, email, display_name, role: 'member', city, state, is_verified: 0, avatar_initial, avatar_color }
      });
              } catch (err) {
