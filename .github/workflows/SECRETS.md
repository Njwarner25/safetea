...
The `mobile-build.yml` workflow needs **9 repository secrets** to produce
...
**What:** Contents of the `.p8` file Apple generated for the API key.
**How to get it:**
...
3. Save the file (e.g., `AuthKey_ABC123XYZ4.p8`)
4. Open it in any text editor — it looks like:
   ```
   -----BEGIN PRIVATE KEY-----
   MIGTAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBHkwdwIBAQQg...
   -----END PRIVATE KEY-----
   ```
5. Copy the **entire file contents including the BEGIN and END lines**
6. Paste as the secret value

### 4. `APPLE_TEAM_ID`
**What:** 10-character Apple Developer team ID (e.g., `9XY8Z7W6V5`).
**How to get it:**
...
**Sanity check:** with these 4 secrets set, the iOS job will use Xcode's
...
**What:** The store password you set during `keytool -genkey`.

### 7. `ANDROID_KEY_ALIAS`
**What:** `safetea-upload` (or whatever alias you used in `keytool -genkey`).
...
| Secret | Without it |
|---|---|
| `APPSTORE_API_KEY_ID` / `APPSTORE_API_ISSUER_ID` / `APPSTORE_API_PRIVATE_KEY` / `APPLE_TEAM_ID` | iOS build runs unsigned, no TestFlight upload |
